...
abstract class CuocoGuiSetting {
	...
	public static class OrdiniCuocoSelezioneListener extends MouseAdapter{		
		@Override
		public void mouseClicked(MouseEvent evt) {
			if (SwingUtilities.isLeftMouseButton(evt) && evt.getClickCount() == 1) {
				int index = CuocoGui.jListOrdini.locationToIndex(evt.getPoint());
				...
				if(index >= 0){
					...
					if (n == 0){
						if (CuocoGui.jListOrdini.getSelectedIndex() != -1) {
							String req = "pub:\n" + Server.UPDATE_CUOCO_ORDINI + "\nNome:" 
									+ CuocoGui.modelloOrdini.getOrdini().get(index).getNomeProdotto() +"\nTavolo:"
									+ CuocoGui.modelloOrdini.getOrdini().get(index).getTavolo()
									+ "\nStato:Fatto\n";

							mandaUpdateAlServer(req);

							CuocoGui.modelloOrdini.ordinePronto(index);
							CuocoGui.jListOrdini.updateUI();
						}
					}
				}

	...
	//ascoltatore pulsante StatoOrdini
		...
		public static class MyButtonInviaListener implements ActionListener {
			public void actionPerformed(ActionEvent evt) {
				...
				if(/*modelloOrdini.getProdotti().get(0).getNome() == "" ||*/ CameriereGui.idCameriereTextField.getText() != "id" /*|| tavoloTextField.getText() == ""*/){
					for(int i=0;i<CameriereGui.modelloOrdini.getSize();i++){
						String req = "pub:\n" + Server.INSERT_CAMERIERE_ORDINI + "\nid:" + CameriereGui.modelloOrdini.getProdotti().get(i).getIdProdotto() 
								+ "\ntavolo:" + CameriereGui.tavoloTextField.getText() + "\nidCameriere:" + CameriereGui.idCameriereTextField.getText() + "\n";
			...
			public void mouseClicked(MouseEvent evt) {
				if (SwingUtilities.isLeftMouseButton(evt) && evt.getClickCount() == 1) {
					...
					int n = JOptionPane.showConfirmDialog(new JFrame(),"Cancellare " 
							+ CameriereGui.modelloOrdini.getProdotti().get(index).getNome() + "?"
